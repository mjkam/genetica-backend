...
class DemoApplicationTests {
	...
	void insertPipeline() {
		...
		Tool bwaTool = Tool.builder()
				...
				.image("bwa image")
				.command("bwa mem -R '@RG\\\\tID:1\\\\tPL:Illumina\\\\tSM:dnk_sample' -t 8 ${reference_fasta} ${input_read_1} ${input_read_2} | samtools view -bS - > ${sample}.bam && samtools index ${sample}.bam > ${sample}_sorted.bam && samtools sort ${sample}_sorted.bam")
		...
		Step step1 = Step.builder()
				...
				.out(Arrays.asList(StepIO.builder().id("output_fasta").build()))
				.run(unTarTool).build();
	...
	void runPipelineTest() {
		...
		request.setPipelineId("5f05d5c2c84cc4062b83a1de");
